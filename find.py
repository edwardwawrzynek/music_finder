...
def download(yt_playlist):
    # download
    result = subprocess.run(["yt-dlp", "-o", "%(playlist_index)s-%(title)s.%(ext)s", "-f", "139", "https://www.youtube.com/watch?list=" + yt_playlist]).returncode
...
def run(yt_playlist, album, artist, music_dir, skip_dir_setup, skip_prompt):
    find_album(yt_playlist, album, artist, music_dir, skip_dir_setup, skip_prompt)
...
run(args.yt_playlist, args.album, args.artist, args.music_dir, args.use_current_dir, args.yes)
